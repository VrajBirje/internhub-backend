...
companySchema.methods.calculateCompletion = function () {
    ...
    if (this.companyName) completion += 25;

    // Step 2: Company Details (25%)
    if (this.description && this.industry && this.websiteUrl && this.companySize) completion += 25;

    // Step 3: Addresses & Contacts (25%)
    if (this.addresses && this.addresses.length > 0 &&
        this.contacts && this.contacts.length > 0) completion += 25;

    // Step 4: Verification (25%)
    if (this.isVerified) completion += 25;
